...
5. **Scene 18 (Dixie Lee at the door) is disconnected from `dixie_lee_editorial`:** ✅ ADDRESSED — added new tri-state `dixie_warmth` ("coffee" / "nod" / "thanks") set in Ch3 scene18 and read at three points: Ch4 scene20 trial prologue, Ch4 scene20 hallway pre-menu, and Ch5 monument (layered alongside the existing `dixie_lee_editorial` block). The emotional arc of the Sabra/Dixie Lee relationship is split between Ch3 scene18 (the coffee/nod choice, affecting `sabra_direction` and `community_standing`) and Ch4 scene20 (the editorial choice, setting `dixie_lee_editorial`). These are resolved in separate variables and never interact — the scene18 choice does not influence scene20 options or the Ch4/Ch5 Dixie Lee callbacks.
